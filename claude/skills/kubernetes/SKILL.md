...
- **`kubectl delete pod` sends SIGTERM** and respects `terminationGracePeriodSeconds`. To simulate an unclean kill (LOCK files, crash-recovery testing), use `kubectl delete pod <name> -n <namespace> --context <ctx> --grace-period=0 --force` (SIGKILL). Source: AKeyRA PR #158 round 5.
