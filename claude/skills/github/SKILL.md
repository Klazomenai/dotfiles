...
- **Request image tag, image digest, and git SHA separately** — `image.tag` (or chart `appVersion`) is a mutable, free-form string that does not uniquely identify a source commit. Tags are human-friendly identifiers, not content addresses — they can be overwritten and may not follow semver. A complete sign-off comment should ask for all three: (1) image tag, (2) image digest from `gcrane digest <image>:<tag>` or equivalent (immutable content reference), (3) source git SHA from the release pipeline's build logs or release metadata — do not use `git rev-parse HEAD` for this, as it records the operator's current checkout, not the commit that produced the image. Source: AKeyRA PR #158 round 5.
