...
- **`grep <key> <chart-file>` for preflight checks is false-positive-prone** — chart comments often mention the same keys as the actual data, causing spurious matches. To extract the actual rendered value, render the chart first and pipe into `yq` (mikefarah/yq, not kislyuk/yq): `helm get manifest <release> -n <ns> | yq eval-all 'select(.kind == "ConfigMap" and .metadata.name == "<name>") | .data["<KEY>"]'`. Selecting only by `.kind` returns the wrong value when a release renders multiple ConfigMaps. Running `yq` directly against a Helm template file does not work — template directives are not rendered and the file may not parse as valid YAML. Source: AKeyRA PR #158 round 7.
...
- `grep <key> <chart-file>` for value extraction — comments cause false positives; use `helm get manifest <release> -n <ns> | yq eval-all 'select(.kind == "<kind>" and .metadata.name == "<name>") | .data["<KEY>"]'` (reads live deployed state) or `helm template <release> <chart> --version <ver> -f values.yaml | yq eval-all 'select(.kind == "<kind>" and .metadata.name == "<name>") | ...'` (pinned version required); selecting by `.kind` alone returns wrong values when a release renders multiple resources of the same kind
